...
- `config/`: Layered YAML defaults under `model/`, `data/`, `optimizer/`, `train/` (merged in order; optional `experiments/` patches). See `training.config_loader.DEFAULT_ERAYZER_CONFIG_REL_PATHS`.
...
- Config defaults load `config/{model,data,optimizer,train}/` then optional extra `--config` files; use `--no-default-config` if you supply a full ordered list.
