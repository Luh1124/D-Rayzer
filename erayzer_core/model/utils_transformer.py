...
from .utils_rope import (
    apply_rope_qk,
    apply_rope_qk_2d_mrpe,
    mrpe_token_positions,
    rope_cos_sin,
)
...
def build_token_norm(d: int, norm_type: str, *, ln_bias: bool = False) -> nn.Module:
    nt = (norm_type or "layernorm").lower()
    if nt == "rmsnorm":
        return RMSNorm(d)
    if nt == "layernorm":
        return nn.LayerNorm(d, bias=ln_bias)
...
class QK_Norm_SelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.rope_mode = (rope_mode or "2d_mrpe").lower()
        self.rope_theta = rope_theta
        self.rope_hh = rope_hh
        self.rope_ww = rope_ww
        self.rope_num_prefix = rope_num_prefix
        if self.use_rope:
            if self.d_head % 2 != 0:
                raise ValueError(f"use_rope requires even d_head, got {self.d_head}")
            if self.rope_mode == "2d_mrpe" and self.d_head % 4 != 0:
                raise ValueError(f"rope_mode=2d_mrpe requires head_dim %% 4 == 0, got {self.d_head}")
            if self.rope_mode == "2d_mrpe" and (rope_hh <= 0 or rope_ww <= 0):
                raise ValueError("rope_mode=2d_mrpe requires positive rope_hh and rope_ww")
            if self.rope_mode not in ("1d", "2d_mrpe"):
                raise ValueError(f"Unknown rope_mode={rope_mode!r}; use '1d' or '2d_mrpe'.")
        ...
        if self.use_rope:
            ...
            if self.rope_mode == "1d":
                cos, sin = rope_cos_sin(
                    seq_len, self.d_head, q.device, q.dtype, self.rope_theta
                )
                q, k = apply_rope_qk(q, k, cos, sin)
            else:
                pos_h, pos_w = mrpe_token_positions(
                    b,
                    seq_len,
                    self.rope_num_prefix,
                    self.rope_hh,
                    self.rope_ww,
                    q.device,
                )
                q, k = apply_rope_qk_2d_mrpe(
                    q, k, pos_h, pos_w, theta=self.rope_theta
                )
...
class QK_Norm_TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = build_token_norm(d, norm_type, ln_bias=ln_bias)
        ...
        self.norm2 = build_token_norm(d, norm_type, ln_bias=ln_bias)
        self.mlp = MLP(d, mlp_ratio, mlp_bias, mlp_dropout)
