...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Extra YAML merged after the default layered stack (repeat allowed).",
    )
    parser.add_argument(
        "--no-default-config",
        action="store_true",
        help="Merge only --config paths (at least one); do not load the default stack.",
    )
    parser.add_argument(
        "--ckpt",
    ...
    parser.add_argument(
        "--server-port", type=int, default=7960, help="Port to bind"
    )
